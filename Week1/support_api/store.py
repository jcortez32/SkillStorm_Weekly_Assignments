...
class restock_manifestNotFoundError(restock_manifestStoreError):
...
def load_manifest(path: Path | None = None) -> tuple[list[RestockItem], list[dict]]:
    ...
    try:
        if path is None:
            resolved_path = Path("data/backupTickets.json")
        raw_text = resolved_path.read_text(encoding="utf-8")
    except FileNotFoundError as e:
        raise restock_manifestNotFoundError(f"No manifest restock data found at {resolved_path}") from e
